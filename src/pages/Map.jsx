...
export default function Map() {
  ...
  useEffect(() => {
    const loadMap = () => {
      if (window.tt && mapRef.current) {
        try {
          ...
          if (!apiKey) {
            console.warn('TomTom API key not found in environment variables');
            showDemoMap();
            return;
          }
          ...
          console.warn('TomTom map loading error:', error);
          showDemoMap();
        }
      } else {
        console.warn('TomTom library not loaded');
        showDemoMap();
      }
    };
    ...
    const showDemoMap = () => {
      if (mapRef.current) {
        // XSS koruması için innerHTML yerine güvenli DOM manipülasyonu
        ...
        title.style.cssText = 'margin-bottom: 1rem; font-weight: bold;';
        title.textContent = 'Demo Harita';
        
        const description = document.createElement('p');
        description.style.cssText = 'margin-bottom: 2rem; opacity: 0.9; max-width: 400px;';
        description.textContent = 'TomTom harita servisi yüklenirken sorun oluştu. Bu alanda interaktif harita görüntülenecektir.';
        ...
        locationDiv.style.cssText = `
          ...
          font-size: 0.9rem;
        `;
        locationDiv.textContent = '📍 Manisa Merkez: 38.6191°N, 27.4280°E';
        
        demoDiv.appendChild(iconDiv);
        demoDiv.appendChild(title);
        demoDiv.appendChild(description);
        demoDiv.appendChild(locationDiv);
        
        mapRef.current.appendChild(demoDiv);
      }
    };
    ...
    const loadTomTom = () => {
      ...
      // JS yükle - Güvenli script yükleme
      if (!window.tt) {
        ...
        script.onerror = () => {
          console.warn('TomTom script failed to load, trying fallback');
          ...
          fallbackScript.onload = () => {
            console.log('TomTom fallback script loaded');
            loadMap();
          };
          fallbackScript.onerror = () => {
            console.warn('TomTom fallback script also failed');
            showDemoMap();
          };
    ...
    const timer = setTimeout(loadTomTom, 100);
